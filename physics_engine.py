...
def step_physics(
    ...
    *,
    ...
    can_drift = abs(state.speed) >= DRIFT_MIN_SPEED

    if (not state.drift_active) and drift_input and can_drift:
    ...
    drift_canceled_for_speed = state.drift_active and not can_drift
    drift_released = state.drift_active and not drift_input
    if drift_canceled_for_speed or drift_released:
        if drift_released and state.drift_charge_frames >= DRIFT_CHARGE_FRAMES:
    ...
    if state.drift_active:
        ...
        steer_for_physics = state.drift_direction
        steer_strength, tuned_skew_degrees = _drift_tuning(
            left_pressed,
            right_pressed,
            state.drift_direction,
        )
        state.drift_skew_degrees = tuned_skew_degrees
    else:
        steer_for_physics = filter_steer_input(steer_input, state.speed)
        steer_strength = 1.0
