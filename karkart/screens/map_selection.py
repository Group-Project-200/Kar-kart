...
# Imported lazily inside handle_event to avoid a circular import at module load
# time (gameplay.py transitively imports a lot of rendering code).


class MapScreen:
    """2x2 grid of map cards with a Back button below."""

    def __init__(self, manager) -> None:
        self.manager = manager

        # 2x2 grid: only four playable tracks are exposed by AppData, so a
        # smaller container gives each card more room without leaving empty
        # slots.
        self.container = MapContainer(
            sp.CENTER_X, sp.CCCBOTTOM,
            sp.WIDTH / 2.5, sp.HEIGHT / 16 * 8,
            rows=2, columns=2,
        )
    ...
    def handle_event(self, event) -> None:
        ...
        if selected_track is not None:
            # Commit the chosen track, then rebuild GamePlay from scratch so
            # every race starts with clean checkpoints, laps, power-ups and
            # car state — even when the user reached this screen via the
            # pause menu's Restart or through Menu -> play again.
