...
class Renderer:
    def __init__(self, current_map : Map, stacker: Stacker, screen):
        self.screen = screen
        render_size = self.build_pixel_surface_size(0.35)
        render_scale = render_size[1] / self.screen.get_size()[1]
        ...
        self.center = (render_size[0] // 2, render_size[1] // 2)
        self.frame_surface = pygame.Surface(render_size).convert()
        self.map = current_map
        self.stacker = stacker
        self.needs_present_scale = render_size != self.screen.get_size()
        self.map.zoom_fixing(draw_map_zoom)
    ...
    def render_frame(self,stack_spread) -> None:
        ...
        self.map.draw_map_camera(display=frame_surface)
